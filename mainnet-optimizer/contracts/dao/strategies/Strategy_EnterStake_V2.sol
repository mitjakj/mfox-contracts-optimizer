...
interface ISingleFarm {
    ...
    function userInfo(address _address) external view returns (uint256);
}

contract Strategy_EnterStake_V2 is Strategy {
    ...
    function balanceOfStakedWant() public override view returns (uint256) {
        if (farmContractAddress != address(0)) {
            uint256 _amount = ISingleFarm(farmContractAddress).userInfo(address(this));
            return _amount;
        } else {
            return 0;
        }
