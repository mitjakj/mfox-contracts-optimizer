...
async function main() {
    ...
    const MAINCHAIN_GAUGE = pools[IS_BLUECHIP ? 'BSC_BLUECHIP' : 'BSC_VOTER'].pool11;
    const LP_TOKEN = addresses.ramsesLP_FBA_USDC;
    ...
    const strategyParams = [
        [
            GAUGE_ADDRESS,
            addresses.ramsesLP_FBA_USDC_GAUGE,
            deployer.address,
        ...
        [
            ...
            addresses.fba, // token0Address
            addresses.usdc  // token1Address
        ],
        true, // isAutoComp
        false, // isStable
        // RAM (v) ETH (v) USDC (v) FBA
        [{from: addresses.ram, to: addresses.weth, stable: false}, {from: addresses.weth, to: addresses.usdc, stable: false}, {from: addresses.usdc, to: addresses.fba, stable: false}],  // earnedToToken0Path
        [{from: addresses.ram, to: addresses.weth, stable: false}, {from: addresses.weth, to: addresses.usdc, stable: false}],  // earnedToToken1Path
        [{from: addresses.fba, to: addresses.usdc, stable: false}, {from: addresses.usdc, to: addresses.weth, stable: false}, {from: addresses.weth, to: addresses.ram, stable: false}],  // token0ToEarnedPath
