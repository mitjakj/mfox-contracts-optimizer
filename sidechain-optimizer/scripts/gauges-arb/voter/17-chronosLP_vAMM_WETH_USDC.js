...
async function main() {
    ...
    const MAINCHAIN_GAUGE = pools[IS_BLUECHIP ? 'BSC_BLUECHIP' : 'BSC_VOTER'].pool61;
    const LP_TOKEN = addresses.chronosLP_vAMM_WETH_USDC;
    ...
    const strategyParams = [
        [
            GAUGE_ADDRESS,
            addresses.chronosLP_vAMM_WETH_USDC_GAUGE,
            deployer.address,
        ...
        true, // isAutoComp
        false, // isStable
        [{from: addresses.chr, to: addresses.weth, stable: false}],  // earnedToToken0Path
        [{from: addresses.chr, to: addresses.usdc, stable: false}],  // earnedToToken1Path
        [{from: addresses.weth, to: addresses.chr, stable: false}],  // token0ToEarnedPath
