...
async function main() {
    ...
    const MAINCHAIN_GAUGE = pools[IS_BLUECHIP ? 'BSC_BLUECHIP' : 'BSC_VOTER'].pool27;
    const LP_TOKEN = addresses.soluneaLP_vAMM_SLNA_WETH;
    ...
    const strategyParams = [
        [
            GAUGE_ADDRESS,
            addresses.soluneaLP_vAMM_SLNA_WETH_GAUGE,
            deployer.address,
        ...
        // SLNA (v) ETH
        [],  // earnedToToken0Path
        [{from: addresses.slna, to: addresses.weth, stable: false}],  // earnedToToken1Path
        [],  // token0ToEarnedPath
        [{from: addresses.weth, to: addresses.slna, stable: false}],  // token1ToEarnedPath
        9990,
